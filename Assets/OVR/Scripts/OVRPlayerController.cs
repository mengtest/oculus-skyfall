...
public class OVRPlayerController : OVRComponent
{
	...
	private AbstractHanger JumpItem;
	...
	private float MoveScaleMultiplier     = 1.5f; 
	...
	public virtual void UpdateMovement()
	{
		...
		if (Input.GetKeyDown(KeyCode.R) || Input.GetButtonDown("ButtonStart")) Reset ();
		if (Input.GetKeyDown(KeyCode.Tab)) SwitchItem ();
		...
		if(Input.GetButtonDown("ButtonA") || Input.GetKeyDown(KeyCode.Space)) Jump ();


		//if(JumpItem != null) {
		if((Input.GetKeyDown(KeyCode.F) || Input.GetButtonDown("ButtonB")) && JumpItem.isUsable) JumpItem.Activate();
			JumpItem.Update();
		//}
	...
	public bool Jump()
	{
		if (!Controller.isGrounded)
			return false;
		FallSpeed = JumpForce;
		//MoveThrottle += new Vector3(0,1, 0);
	...
	public void OnControllerColliderHit(ControllerColliderHit hit) {
		...
		if(!JumpItem.OnBeforeControllerColliderHit(hit)) return;

		if(FallSpeed < -1 && hit.moveDirection.y < -0.3f) {
			this.Die ();
		}
		...
		JumpItem.OnAfterControllerColliderHit(hit);
	}
